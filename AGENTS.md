...
- The Dashboard uses an existing local device capture. The requested `C:\AirLink screenshot` folder was absent during initial delivery, so Settings and Statistics are deliberate monochrome previews in `index.html`; replace them with real, scrubbed product screenshots in `assets/screenshots/` when supplied. Do not use customer or payment data in public images.
